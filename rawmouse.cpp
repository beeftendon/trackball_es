			...
			if (raw_mice[i].device_handle == raw->header.hDevice)
			{
				...
				if (include_sys_mouse) { 
					if (raw_mice[i].is_absolute) {
						...
						raw_mice[RAW_SYS_MOUSE].x += raw->data.mouse.lLastX;
						raw_mice[RAW_SYS_MOUSE].y += raw->data.mouse.lLastY;
						/*currx += raw->data.mouse.lLastX;
						curry += raw->data.mouse.lLastY;*/
					}
...
/*
void get_xypos(int * mx, int * my)  // takes pointers to integers x and y -- make this return dx, dy, dtheta since last call; each of those accumulates...
{
	*mx=currx;    // sets values pointed to by those pointers
	*my=curry;
}
*/
